...
import BCQ
import BCQREM
import BCQREM_share_Qparam
import BCQREM_share_VQparam
import smallDDPG
import utils

'''
from https://github.com/sfujim/BCQ
'''
...
def interact_with_environment(env, state_dim, action_dim, max_action, device, args):
	...
	policy = smallDDPG.DDPG(state_dim, action_dim, max_action, device)#, args.discount, args.tau)
...
def train_BCQ(state_dim, action_dim, max_action, device, args):
	...
	if args.model == 'BCQ':
		...
		policy = BCQREM.BCQ(state_dim, action_dim, max_action, device, args.discount, args.tau, args.lmbda, args.phi)
	elif args.model == 'BCQREMshareQ':
		policy = BCQREM_share_Qparam.BCQ(state_dim, action_dim, max_action, device, args.discount, args.tau, args.lmbda, args.phi)
	elif args.model == 'BCQREMshareVQ':
		policy = BCQREM_share_VQparam.BCQ(state_dim, action_dim, max_action, device, args.discount, args.tau, args.lmbda, args.phi)
	# Load buffer
	replay_buffer = utils.ReplayBuffer(state_dim, action_dim, device)
	...
	while training_iters < args.max_timesteps: 
		vae_loss, actor_loss, critic_loss = policy.train(replay_buffer, iterations=int(args.eval_freq), batch_size=args.batch_size)
...
if __name__ == "__main__":
	...
	parser.add_argument("--seed", default=2021, type=int)              # Sets Gym, PyTorch and Numpy seeds
	...
	parser.add_argument("--model", default="BCQ")   # BCQREM, BCQREMshareQ, BCQREMshareVQ
