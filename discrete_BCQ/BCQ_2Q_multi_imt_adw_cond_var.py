...
class Conv_Q(nn.Module):
	...
	def forward(self, state, need_std=False):
		...
		return q2_1, q2_2, F.log_softmax(i, dim=1), i, v
...
class discrete_BCQ(object):
	...
	def train(self, replay_buffer):
		...
		current_Q1, current_Q2, train_imt, train_i, current_value = self.Q(state, need_std=False)
		...
		with torch.no_grad():
			...
			target_Q1, target_Q2, imt, i, target_state_value = self.Q_target(next_state, need_std=False)
			# Soft Clipped Double Q-learning 
			target_Q = self.lmbda * torch.min(target_Q1, target_Q2) + (1. - self.lmbda) * torch.max(target_Q1, target_Q2)
			target_Q = target_Q.gather(1, next_action).reshape(-1, 1)

			# the target of Q function
