...
class Conv_Q(nn.Module):
	...
	def compute_Q(self, state, need_std=False):
		...
		q2 = self.q2(q)	# compute Q-values (batch, action dim)
...
class discrete_BCQ(object):
	...
	def select_action(self, state, eval=False):
		...
		if np.random.uniform(0,1) > self.eval_eps:
			with torch.no_grad():
				...
				q, imt, i,  _ = self.Q.compute_Q(state, need_std=False)	# use average Q-values during the test
				imt = imt.exp()
	...
	def train(self, replay_buffer):
		...
		with torch.no_grad():
			...
			q, imt, i, _ = self.Q.compute_Q(next_state, need_std=True)
			...
			q, imt, i, target_state_value = self.Q_target.compute_Q(next_state, need_std=False)
