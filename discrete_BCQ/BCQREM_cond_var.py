...
class discrete_BCQ(object):
	...
	def _pick_data(self, std_matrix, current_Q, next_state, reward, done):
		# pick current_Q, next_state, reward, done  TODO
		# next action std
		# next_action_std = (std_matrix/std_matrix.max(1, keepdim=True)[0])
		print(std_matrix)
		next_action_std = (std_matrix/std_matrix.max(1, keepdim=True)[0] < self.std_threshold).float()
		
		print(next_action_std.shape)
		print(next_action_std)
		pick_index, _ = torch.where(next_action_std != 0.0)
		print(pick_index.shape)
		print(pick_index)

		# pick_index, _ = torch.where(next_action_std  < self.std_threshold)
		# pick_state = torch.index_select(state, dim=0, index=pick_index)

		# # Use large positive number to mask action_std from argmin
		# next_action_std = (next_action_std * std_matrix + (1 - next_action_std) * 1e8).argmin(1, keepdim=True)
		# print(next_action_std)
		assert 0>1
		return 
	...
	def train(self, replay_buffer):
		...
		current_Q, train_imt, train_i, _ = self.Q(state, need_std=False)
		current_Q = current_Q.gather(1, action)

		# Compute the target Q value
		with torch.no_grad():
			# use average Q-values during no_grad
			q, imt, i, std_matrix = self.Q.compute_Q(next_state, need_std=True)

			# TODO   Probability / Var or Probability / Std
			# imt = imt.exp()
