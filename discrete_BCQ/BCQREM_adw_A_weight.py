...
class discrete_BCQ(object):
	...
	def train(self, replay_buffer):
		...
		with torch.no_grad():
			...
			q, next_imt, i, next_std_matrix, target_state_value = self.Q_target.compute_Q(next_state, need_std=True)
			...
			A_matrix = next_imt.exp() / (next_std_matrix + self.EPS)		# prob / var
			A_weight = A_matrix.gather(1, next_action)
		...
		# A weight and advantage weight
		i_loss = (A_weight * i_loss * torch.exp(current_Q.reshape(-1) - current_value.reshape(-1))).mean()
